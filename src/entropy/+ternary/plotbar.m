...
if nargin >  3
    switch class(d)
        ...
        case {'double','int'}
            ...
            if (ishold() && any(arrayfun(@(dat) any(dat < -eps) || any(dat > 1.0),d)))
...
%    if not(ishold())%Draw the colorbar on the side!!
    ...
    for i=1:M
        [height,kk] = size(map);
        if intc
            in=1+round((d(i)-miv)*(height)/(mav-miv+1));
        else
            in=1+round((d(i)-miv)*(height)/(mav-miv));
        end
        %bound the in coordinate within the map index
        if in==0, in=1; end
        if in > height, in=height;end
        %numcolors = length(in);%FVA: uncommented 20/01/2014 to see if it solves bug
        %q=plot(x(i),y(i),'s','Color',map(in,:),'markerfacecolor',map(in,:),varargin{:});
        % Check that none of the values around here are NaNs or else it
        % will refuse to plot noisily!
        q=plot(x(i),y(i),'s','Color',map(in,:),'MarkerFaceColor',map(in,:),varargin{:});
    end
