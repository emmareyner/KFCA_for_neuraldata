function [tildeK_R_T,tildeK_R_E,memory]=dilative(X_E,X_T,K_R_E,K_R_T,options)
% function [tildeK_R_T,tildeK_R_E,memory] = mlc.hmams.dilative(X_E,Y_E,X_T)
%
...
%Y_E_log = mmp.x.Sparse(log(+(full(task.K_R_E.I.'))));%Initial version ALL
%INF for dilative memories!
% Y_E_log = +(full(task.K_R_E.I.'));%Second version
% Y_E_log(Y_E_log == 0) = -1;
% Y_E_log = Y_E_log * 1;
%R_E_log_inv = mmp.n.Sparse(-log(+(full(R_E.'))));
Y_E_log = mmp.x.Sparse(-log(+(full(K_R_E.I.'))));%Third version

%dilation = mmp.l.mrdivide(R_E_log,I_D_E);%Maybe it needs R_E to be min-plus encoded!!
memory = hmams.dilative.create(X_E,Y_E_log);
...
if (nargout > 1)
    tildeK_R_E = fca.mmp.Context(...
        K_R_E.G,K_R_E.M,...
        (hmams.dilative.recover(memory,X_E)).',...
        sprintf('%s-%s',K_R_E.Name,'dilative-hmam'));
end

%explore the different values of either type of memory!
tildeK_R_T = fca.mmp.Context(...
    K_R_T.G,K_R_T.M,...
    hmams.dilative.recover(memory,X_T).',...
    sprintf('%s-%s',K_R_T.Name,'dilative-hmam'));
return
