function [Sp,Aplus] = of_UFNF_1(A,comps,adj,cycles)
% function [Sp,Aplus] = mmp.l.Spectrum.of_UFNF_1(A,comps,adj,cycles)
%
...
% mmp.l.Spectrum.of_UFNF_0, digraph.crc_scc_cycles.m
% mmp.l.Spectrum.all_evs.m, mmp.l.Spectrum.UtSV
%
% Author: fva 03/12/07
% Doc:    fva 10/01/08

%The representational invariance is that S.embedded is a list of nC
%strongly connected components of type mmp.l.Spectrum0
...
if isa(A,'double')
    if issparse(A)
        error('mmp:l:Spectrum:of_UFNF_1','Direct sparse input not allowed to prevent formatting confusions. Use mmp.x.Sparse, etc.');
    else
        A = mmp.x.Sparse(A);
    end
elseif isa(A,'mmp.x.Sparse')%MX sparse encoding OK
    ...
    error('mmp:l:Spectrum:of_UFNF_1','Wrong input class');
end
%From now on A is mmp_x_sparse!!
%spA = true;%FVA: KLUDGE

if not(any(any(adj)))%empty adjacency. Completely disconnected subdigraph. Do not process here!
    error('mmp:l:Spectrum:of_UFNF_1','Completely disconnected digraph');%Sp.order = adj;
elseif any(any(tril(adj,-1)))%Sanity check: The graph is not in (upper) Frobenius normal form.
    error('mmp:l:Spectrum:of_UFNF_1','Not a component in UFNF!');
elseif not(all(diag(adj)))%some component has no cycle
    error('mmp:l:Spectrum:of_UFNF_1','Some of the components is not strongly connected!');
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Sp.embedded=cell(1,nC);%Spectrum for each scc
...
for c=1:nC
    ...
    %Get the spectra of irreducible components
    %[Sp.lambdas(c),metric_matrices{c},ccindex] =...
    Sp0 = mmp.l.Spectrum.of_UFNF_0(A(compsc,compsc),mlcycles);
    Sp.embedded{c}=Sp0;
    % The cycles are expressed relative to the local matrix, so ...
    %change back to global numbering
    Sp.ccycles{c}={cyclesc{Sp0.ccindex}};
    Sp.lambdas{c}=Sp0.lambda;
