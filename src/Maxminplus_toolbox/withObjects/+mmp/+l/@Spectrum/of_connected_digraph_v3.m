...
%         %regular suppor for right eigenvalues comes from upstream non-dominating classes
%         rsupp = usc & ~dom_usc;
%         lsupp = dsc & ~dom_dsc;%reg. support for left eigenvectors: downstream non-dominating classes
        ...
        dsc(c)=false;
        if any(dsc)
            dgnsupp=[comps{dsc}];%downwards global node support
            if any(dom_dsc)
                B=mmp.l.zeros(sizeA);%However big, it is mostly empty!
                %create block diagonal Inf mask
                for satc=find(dom_dsc)
                    satsupp = comps{satc};
                    B(satsupp,satsupp)=Inf;
                end
                %star of finitely & saturatedly supporting classes, proven to exist
                lnStar = mmp.l.plus(A(dgnsupp,dgnsupp),B(dgnsupp,dgnsupp));
            else
                %star of finitely supporting classes, proven to exist
                lnStar = A(dgnsupp,dgnsupp);
            end
            lnSat=mmp.l.trclosure(mmp.l.mrdivide(lnStar,norm))
            lnStar=mmp.l.finite_trclosure(mmp.l.mrdivide(lnStar,norm));
            llink = mmp.l.mrdivide(A(compsc,dgnsupp),norm);
            lpat(:,Sp.g2l(dgnsupp))=mmp.l.mtimes(mmp.l.rclosure(metric_matrices{c}),mmp.l.mtimes( llink, lnStar));
        end
        %The values for the finite support of this class
