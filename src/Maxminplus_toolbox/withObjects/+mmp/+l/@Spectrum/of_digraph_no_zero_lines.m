function [spectra,subdigraphs,orders, Aplus] = of_digraph_no_zero_lines(A)
% function [spectra,subdigraphs,orders,Aplus] = mmp.l.Spectrum.of_digraph_no_zero_lines(A)
% function [spectra,subdigraphs,orders] = mmp.l.Spectrum.of_digraph_no_zero_lines(A)
%
...
if isa(A,'double')
    if spA
        error('mmp:l:Spectrum:of_digraph_no_zero_lines','Direct sparse input not allowed to prevent formatting confusions. Use mmp.x.Sparse, etc.');
    else
...
for s = 1:nS
    if ~any(any(orders{s})),
        error('mmp:l:Spectrum:of_digraph_no_zero_lines','Disconnected components! Some rows or columns are null!');
    end
end
% is_connected = false(1,nS);
% for s=1:nS, is_connected(s) = any(any(orders{s})); end
% if any(~is_connected(s))
%     error('mmp:l:Spectrum:of_digraph_no_zero_lines','Some rows or columns are null!');
% end
...
if Aplusrequested
    Aplus=cell(1,nS);
    for s=1:nS
        [spectra{s},Aplus{s}]=mmp.l.Spectrum.of_connected_digraph(A,subdigraphs{s},orders{s},allcycles{s});
    end
else%Aplus not requested
    for s=1:nS
        spectra{s}=mmp.l.Spectrum.of_connected_digraph(A,subdigraphs{s},orders{s},allcycles{s});
    end
end
