function [Lev,lambdas,Len,Rev,rhos,Ren,P3,Aplus] = of_digraph(A,rmask,cmask)
%function [Lev,lambdas,Len,Rev,rhos,Ren,P3,Aplus] = mmp.l.Spectrum.of_digraph(A,rmask,cmask)
%
...
% mmp.l.Spectrum.of_digraph_no_zero_lines and
...
Aplusrequested =nargout >7;
if Aplusrequested, Aplus=mmp.l.zeros(m); end%Build the Aplus, just in case

%find zero rows and cols to see if UFNF_2 is possible
[zrows,zcols]=findzeros(A(rmask,cmask));
if ~any(zrows|zcols)%Accepts an UFNF_2
        if Aplusrequested%Only work out the transitive closure if requested.
            [SSPP, subgraphs, orders,AApp] = mmp.l.Spectrum.of_digraph_no_zero_lines(A);
            wsupp=0;
            for s = 1:length(AApp)
                m=size(AApp{s},1);
                ran=wsupp+(1:m);
            ...
            [SSPP, subgraphs, orders] = mmp.l.Spectrum.of_digraph_no_zero_lines(A);
    ...
    if Nb ~=0%a cyclic part
        ...
        if Aplusrequested
            [Levb,lambdasb,Lenb,Revb,rhosb,Renb,nodesb,Aplusembedded] = mmp.l.Spectrum.of_digraph(A(Vb,Vb));
            Aplus(nodesb,nodesb)=Aplusembedded;
        else
            [Levb,lambdasb,Lenb,Revb,rhosb,Renb,nodesb] = mmp.l.Spectrum.of_digraph(A(Vb,Vb));
        end
        
        P3=[find(Vi) find(Va) nodesb find(Vw)];%global permutation
        ...
        %full(mmp.l.mtimes(Lev,A(P3,P3))==mmp.l.mtimes(mmp.l.diag(lambdas),Lev))
         ...
         P3=[find(Vi) find(Va) find(Vb) find(Vw)];%Complete permutation
    end
end
