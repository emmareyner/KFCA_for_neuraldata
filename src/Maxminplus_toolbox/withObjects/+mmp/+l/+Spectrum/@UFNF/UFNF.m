classdef UFNF
    % An abstract class to represent the spectrum of a maxplus matrix.
    properties
...
%         ec_above=logical([]);%wheter sccs it is eclipsed the left spectrum
        lambdas=[];%left spectrum
        rhos = [];%right spectrum
        renodes={};%right eigen-nodes
        lenodes={};%left eigen-nodes
    end
    methods
        function [S]= UFNF(A,maskperm)
            switch nargin
                case 1%A is supposed to be square already
                    maskperm = 1:size(A,1);%null permutation, identity map
                case 2
                    if ~isnumeric(maskperm)
                        warning('mmp:l:Spectrum:UFNF','non-permutation mask');
                        maskperm=find(maskperm);
                    end
                otherwise
                    error(nargchk(1,2,nargin))
            end
            %build local to global map and permutation
            S.nodes = maskperm;
            %Build global to local map
            k=length(maskperm);
            m=max(maskperm);
            S.g2l=sparse(1,m);%empty map
            S.g2l(maskperm)=1:k;%Global to local map
        end%UFNF
    end
    methods (Abstract)
