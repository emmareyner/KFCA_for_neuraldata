function Rstar = finite_trclosure(R)
...
if isa(R,'double')
    Rstar = mmp_l_finite_trclosure(R);
elseif isa(R,'mmp.x.Sparse')
    Rstar = finite_trclosure(R);
elseif isa(R,'mmp.n.Sparse')
    %CAVEAT: this might be really costly
    %what we need here is PROBABLY, the transitive reduction in
    %mmp.n.Sparse format
    Rstar = mmp_l_finite_trclosure(double(R));
else
    error('mmp:l:trclosure','wrong input data type. %s',class(R))
end
