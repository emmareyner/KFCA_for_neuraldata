...
% TODO: iterative version of algorithm
    Rstar = mmp.x.Sparse();
    Rstar.Reg = mmp_l_xXxZ_finite_trclosure(R.Reg);
%Rstar = mmp.x.Sparse(mmp_l_xXxZ_finite_trclosure(R.Reg));
% [m n]=size(R);
% if m~=n
%     error('mmp:x:Sparse:finite_trclosure','Not a square matrix');
% end
% if m > 1
%     Rstar = mmp.x.Sparse();
%     Rstar.Reg = mmp_l_xXxZ_finite_trclosure(R.Reg);
...
% else%if m == 0
%     error('mmp:x:Sparse:finite_trclosure','undefined on empty matrix');
% end
return%star
