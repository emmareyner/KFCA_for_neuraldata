classdef Vizgraph < handle
    ...
    % Vizgraph('-adjMat',adj,'-nodeLabels',labels,'-nodeColors',colors);
    %
    ...
    %                         color. If you do not label the nodes, Vizgraph
    ...
    % s = Vizgraph('-adjMat',adj,'-nodeLabels',labels,'-nodeColors',colors);
    % freeze(s); % convert to an image
    %
    % If you are only specifying an adjacency matrix, you can omit the
    % '-adjMat' name as in Vizgraph(adj).
    %
    % Calling Vizgraph without any parameters displays an example graph.
    %
    ...
    properties(GetAccess = 'public', SetAccess = 'private')
        ...
        graphVizPath = network.Vizgraph.setupPath();
        ...
        knownLayouts = {network.Gvizlayout()  ,...   % add your own layout here or use
            network.Treelayout()  ,...   % the addLayout() method
            network.Radiallayout,...
            network.Circularlayout,...
            network.Springlayout,...
            network.Circlelayout,...
            network.Gridlayout  ,...
            network.Randlayout  };
    ...
    methods
        ...
        function obj = Vizgraph(varargin)
            % Vizgraph constructor
            %if(~exist('my.processArgs','file')), error('network:Vizgraph','Requires processArgs() function');            end
            obj.addKnownLayouts();
        ...
        function addLayout(obj,layout)
            ...
            if(~ismember(layout.name,fieldnames(obj.layouts)))
                if(layout.isavailable())
                    ...
                    warning('Vizgraph:layout','This layout is not available');
                end
            end
        end
        ...
        function setLayout(obj,layout)
            ...
            if(layout.isavailable())
                ...
                warning('Vizgraph:setLayout','Sorry, this layout is not available');
            end
        end
    ...
    methods(Access = 'protected')
        ...
        function processInputs(obj,varargin)
            ...
            [adjMatrix, currentLayout, nodeLabels, nodeDescriptions, nodeColors,obj.undirected,obj.edgeColors,obj.splitLabels,obj.doubleClickFn] = my.processArgs(varargin,...
                '-adjMat'               , []     ,...
                '-layout'               , []     ,...
                '-nodeLabels'           , {}     ,...
                '-nodeDescriptions'     , {}     ,...
            ...
            if(~isequal(numel(obj.nodeLabels),size(adjMatrix,1),size(adjMatrix,2)))
                error('Vizgraph:dimMismatch','The number of labels must match the dimensions of adjmatrix.');
            end
        ...
        function addNodes(obj)
            ...
            obj.nodeArray = [];
            for i=1:obj.nnodes
                newnode = network.VizgraphNode(obj.nodeLabels{i});
...
%             %%%% FVA: this does not work for MAXMINPLUS!!!!
...
%             end
            ...
            for i=1:obj.nnodes
                for j=1:obj.nnodes
                    if(obj.adjMatrix(i,j))%FVA: this needs to change for each type of visualization
