function [Adj, labels, x, y] = from_dot(filename)
% [Adj, labels, x, y] = digraph.from_dot(filename)
...
%       Complains about RecursionLimit on huge graphs.
...
for line_ndx = 1:Nlns          % This section sets the adjacency matrix entry A(Lnode,Rnode) = edge_id.
    ...
    for dash_pos = [Ddash_pos arrow_pos];  % if empty - not a POS line
        ...
        unread = my.setdiff(unread, line_ndx); 
    end
end
