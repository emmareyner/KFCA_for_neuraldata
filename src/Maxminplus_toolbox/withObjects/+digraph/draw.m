function [x, y, labels] = draw(adj, labels)
%
% [x, y, labels] = draw_dot(adj, labels)   draw a graph defined by adjacency matrix 
%  
% Sample code illustrating use of digraph.to_dot and digraph.from_dot.m functions
%     for interfacing  GraphViz layout and Matlab UI powers  
%
% (C) Dr. Leon Peshkin  pesha @ ai.mit.edu  /~pesha     24 Feb 2004
%
% Modified by FVA to be included in the +digraph package OCT-NOV 2010

%% Input data massaging
[n,m] = size(adj);
if n ~= m
    warning('digraph:examples:draw_dot','not a square adjacency matrix!')
end
directed = ~(isequal(triu(adj,1),tril(adj,-1)'));%detect whether it is a digraph
% if isequal(triu(adj,1),tril(adj,-1)')
%     directed = 0;
% else
%     directed = 1;
% end 
...
digraph.drawDot(tmpLAYOUT)
%% load back into matlab 
%[trash, names, x, y] = dot_to_graph(tmpLAYOUT);  % load NEATO layout
[trash, names, x, y] = digraph.from_dot(tmpLAYOUT);  % load NEATO layout
num_names = str2num(char(names))'; %#ok<ST2NM>
nam_len = length(names);
if nam_len < n  % plot singletons without coordinates all together in a lower left 
    num_names(nam_len+1:n) = setdiff(1:n, num_names);
...
delete(tmpLAYOUT);
delete(tmpDOTfile);     % clean up temporary files
return
