...
%% PROCESS VARARGIN - PASSED BY PROGRAMMER
    ...
    if ~iscell(userArgs)
        throwAsCaller(MException('PROCESSARGS:noUserArgs',...
            'PROGRAMMER ERROR - you must pass in the user''s arguments in a cell array as in processArgs(varargin,''-name'',val,...)'));
    end
    if isempty(varargin)
        throwAsCaller(MException('PROCESSARGS:emptyVarargin',...
            'PROGRAMMER ERROR - you have not passed in any name/default pairs to processArgs'));
    end
    ...
    if ~iscellstr(progArgNames),
        throwAsCaller(MException('PROCESSARGS:notCellStr ',sprintf('PROGRAMMER ERROR - you must pass to processArgs name/default pairs')));
    end
    %% Remove * and + Markers
    try
    ...
    if mod(numel(varargin),2)
        throwAsCaller(MException('PROCESSARGS:oddNumArgs',...
            sprintf('PROGRAMMER ERROR - you have passed in an odd number of arguments to processArgs, which requires name/default pairs')));
    end
    if any(cellfun(@isempty,progArgNames))
        throwAsCaller(MException('PROCESSARGS:emptyStrName ',sprintf('PROGRAMMER ERROR - empty-string names are not allowed')));
    end
    if nargout ~= 1 && nargout ~= maxNargs
        throwAsCaller(MException('PROCESSARGS:wrongNumOutputs',...
            sprintf('PROGRAMMER ERROR - processArgs requires the same number of output arguments as named/default input pairs')));
    end
    if ~isempty(PREFIX) && ~all(cellfun(@(c)~isempty(c) && c(1)==PREFIX,progArgNames))
        throwAsCaller(MException('PROCESSARGS:missingPrefix',...
            sprintf('PROGRAMMER ERROR - processArgs requires that each argument name begin with the prefix %s',PREFIX))); 
    end
    if FULL_ERROR_CHECK  && numel(unique(progArgNames)) ~= numel(progArgNames)
        throwAsCaller(MException('PROCESSARGS:duplicateName',sprintf('PROGRAMMER ERROR - you can not use the same argument name twice')));
    end
%% PROCESS USERARGS
    ...
    if numel(userArgs) == 0 && nargout > 1
        if any(required)
            throwAsCaller(MException('PROCESSARGS:missingReqArgs',...
                sprintf('The following required arguments were not specified:\n%s',catString(progArgNames(required))))); 
        else
            return;
        end
    end
    if FULL_ERROR_CHECK 
     ...
     % checks for missing '-' 
        if ~isempty(PREFIX)
            userstrings = lower(userArgs(cellfun(@(c)ischar(c) && size(c,1)==1,userArgs)));
            problem = ismember(userstrings,cellfuncell(@(c)c(2:end),progArgNames));
            if any(problem)
                if sum(problem) == 1
                    warning('processArgs:missingPrefix',...
                        'The specified value ''%s'', matches an argument name, except for a missing prefix %s. It will be interpreted as a value, not a name.',...
                        userstrings{problem},PREFIX)
                else
                    warning('processArgs:missingPrefix',...
                        'The following values match an argument name, except for missing prefixes %s:\n\n%s\n\nThey will be interpreted as values, not names.',PREFIX,catString(userstrings(problem)));
                end
            end
        end
    end
    %% Find User Arg Names
    userArgNamesNDX = find(cellfun(@(c)ischar(c)  && ~isempty(c) && c(1)==PREFIX,userArgs));
    %% Check User Arg Names                                  
    if ~isempty(userArgNamesNDX) && ~isequal(userArgNamesNDX,userArgNamesNDX(1):2:numel(userArgs)-1)
        if isempty(PREFIX)
            throwAsCaller(MException('PROCESSARGS:missingVal',...
                sprintf('\n(1) every named argument must be followed by its value\n(2) no positional argument may be used after the first named argument\n')));
        else
            throwAsCaller(MException('PROCESSARGS:posArgAfterNamedArg',...
                sprintf('\n(1) every named argument must be followed by its value\n(2) no positional argument may be used after the first named argument\n(3) every argument name must begin with the ''%s'' character\n(4) values cannot be strings beginning with the %s character\n',PREFIX,PREFIX)));
        end
    end
     if FULL_ERROR_CHECK  && ~isempty(userArgNamesNDX) && numel(unique(userArgs(userArgNamesNDX))) ~= numel(userArgNamesNDX)
    ...
    if numel(positionalArgs) + numel(userArgNamesNDX) > maxNargs
        throwAsCaller(MException('PROCESSARGS:tooManyInputs',...
            sprintf('You have specified %d too many arguments to the function',numel(positionalArgs)+numel(userArgNamesNDX)- maxNargs)));
    end
    ...
    % empty, i.e. '',{},[]    
        if ~isempty(userArgs{i})  
           argsProvided(i) = true;
           if typecheck(i) && ~isa(userArgs{i},class(defaults{i}))
               throwAsCaller(MException('PROCESSARGS:argWrongType',sprintf('Argument %d must be of type %s',i,class(defaults{i}))));
           end
    ...
    %% Error Check User Args
    if any(~userProgMap)
        throwAsCaller(MException('PROCESSARGS:invalidArgNames',...
            sprintf('The following argument names are invalid: %s',catString(userArgNames(userProgMap == 0),' , '))));
    end
    if any(userProgMap  <= numel(positionalArgs))
        throwAsCaller(MException('PROCESSARGS:bothPosAndName' ,...
            sprintf('You cannot specify an argument positionally, and by name in the same function call.')));
    end
    %% Extract User Values
    ...
    if any(typecheck)
       for i=1:numel(userArgNamesNDX)
          if typecheck(userProgMap(i)) &&  ~isa(userArgs{userArgNamesNDX(i)+1}, class(defaults{userProgMap(i)}))
            throwAsCaller(MException('PROCESSARGS:wrongType',...
                sprintf('Argument %s must be of type %s',userArgs{userArgNamesNDX(i)},class(defaults{userProgMap(i)})))); 
          end
       end
    end    
    ...
    if any(~argsProvided & required) 
        throwAsCaller(MException('PROCESSARGS:emptyVals',...
            sprintf('The following required arguments were either not specified, or were given empty values:\n%s',catString(progArgNames(~argsProvided & required)))));
    end
    %% Relay Mode
